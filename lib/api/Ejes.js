    ...
    class Ejes {
        static init(session, ip, ep, debug = 0) {
            ...
            if (debug & (session.DEBUG_REQUEST | session.DEBUG_FETCH_METADATA)) {
                session.log("*** DEBUG *** [" + (new Date()).valueOf() + "] Ejes:init - Request data=" + payload + ", Query=" + Ejes.EJES_INIT);
            }
        ...
        static cancelDownload(session, debug = 0) {
            if (debug & session.DEBUG_REQUEST) {
                session.log("*** DEBUG *** [" + (new Date()).valueOf() + "] Ejes:cancelDownload - No request data.  Query=" + Ejes.EJES_CANCEL_DOWNLOAD);
            }
        ...
        static exec(session, ep, debug = 0) {
            ...
            if (debug & (session.DEBUG_REQUEST | session.DEBUG_FETCH_METADATA)) {
                session.log("*** DEBUG *** [" + (new Date()).valueOf() + "] Ejes:exec - Request data=" + payload + ", Query = " + Ejes.EJES_EXEC);
            }
            return imperative_1.RestClient.postExpectJSON(session, Ejes.EJES_EXEC, [], payload);
        }
        static term(session, debug = 0) {
            if (debug & session.DEBUG_REQUEST) {
                session.log("*** DEBUG *** [" + (new Date()).valueOf() + "] Ejes:term - No Request Data.  Query=" + Ejes.EJES_TERM);
            }
            return imperative_1.RestClient.postExpectJSON(session, Ejes.EJES_TERM);
        }
    }
    Ejes.EJES_INIT = "/init?q=loginfo,environment,version,function,find,notice,position,message,lines";
    Ejes.EJES_CANCEL_DOWNLOAD = "/cancel-download";
    Ejes.EJES_EXEC = "/exec?q=loginfo,position,find,notice,message,lines";
    Ejes.EJES_TERM = "/term";
    return Ejes;
})();
exports.Ejes = Ejes;
