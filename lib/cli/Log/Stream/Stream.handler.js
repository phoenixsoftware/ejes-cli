...
class ListHandler {
    process(params) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let limitMinutesMax; // --lines time
            let limitLinesMax; // --lines count
            ...
            const logoff = () => __awaiter(this, void 0, void 0, function* () {
                ...
                try {
                    ...
                    try {
                        require("fs").writeFileSync(".\\ejesLogStreamTerminationDiagnostics.txt", util.inspect(e, this.inspectOptions()));
                    }
                    catch (err) {
                        this.session.error("Error writing .\\ejesLogStreamTerminationDiagnostics.txt: " + util.inspect(err));
                    }
                    if (!reason) {
            ...
            const messageFilterOptionParse = (option, model, fieldLength, flags) => {
                const parm = params.arguments[option];
                let list = "";
            ...
            const doFindHilite = (response, line, index, start = "[", stop = "]") => {
            ...
            const isRefreshSetup = (response, index) => {
            ...
            const debugOption16Blocking = (response, index, countOfLinesOutput, isFirstRecord) => {
                let debug = "";
            ...
            */
            ...
            const lineOutputComplete = (response, index, contextCounter) => {
            ...
            const enumerationComplete = (response) => {
            ...
            const fetchData = () => __awaiter(this, void 0, void 0, function* () {
                ...
                do {
                    try {
                        ...
                        try {
                            require("fs").writeFileSync(".\\ejesLogStreamDiagnostics.txt", util.inspect(e, this.inspectOptions()));
                        }
                        catch (err) {
                            this.session.error("Error writing .\\ejesLogStreamDiagnostics.txt: " + util.inspect(err));
                        }
                        try {
                            ...
                            try {
                                msg = (ip ? "Initialization" : "Execution") + " request: " + e.mDetails.msg + ". " + (e.mDetails.causeErrors ? e.mDetails.causeErrors + "." : "") + " Diagnostics in ejesLogStreamDiagnostics.txt.";
                            }
                            catch (e3) {
    ...
    debugResponse(level, tag, resp) {
        if (resp.lines)
            resp.count = resp.lines.length;
        if (resp.message && resp.message.longMessages.length > 0)
            resp.error = resp.message.longMessages[0] + " (" + resp.message.shortMessage + ")";
        if (resp.position)
            resp.isVerticalAdjustment = resp.position.isVerticalAdjustment;
