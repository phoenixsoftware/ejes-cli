...
import { IEjes, IScreen } from "../../../api/Doc/IEjes";
...
export default class ListHandler implements ICommandHandler {
    ...
    public async process(params: IHandlerParameters): Promise<void> {
        ...
        let limitLinesMax : number;         // --limit
        let limitMinutesMax : number;
        let onlyMinutesMax : number;
        let onlyEnumerateMax : number;      // --only
        let countEnumeratedLines : number;  // --only
        ...
        const findAndMark = (response: IEjes, line: string, index: number, start: string = "[", stop: string = "]") : string => {
        ...
        const debugOption16Blocking = (response: IEjes, index: number, countOfLinesOutput: number) : string => {
            let debug = "";
        ...
        const reasonReturn = (isReason: boolean, reasonText) => {
            if ( isReason )
                reason = reasonText;
            return isReason;
        };
        ...
        const lineOutputComplete = (response: IEjes, index: number, countOfLinesOutput: number) : boolean => {
        ...
        const enumerationComplete = (response: IEjes, countOfLinesOutput: number) : boolean => {
